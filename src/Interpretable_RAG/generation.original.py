    ...
    def from_pretrained(pretrained_model_name_or_path:str, *args, **kwargs) -> 'ExplainableAutoModelForGeneration':
        ...
        class _ExplainableAutoModelForGeneration(T, ExplainableAutoModelForGeneration):
            ...
            def forward(self, *args, **kwargs):
                ...
                if self._explain: self._exp_logits[-1].append(outputs.logits[:,-1:,:].detach())
                else:             self._gen_logits.append(outputs.logits[:,-1:,:].detach())
            ...
            def generate(self, inputs:List[str], **kwargs) -> List[str]:
                ...
                self._gen_logits  = torch.concatenate(self._gen_logits, dim=1).cpu()
            ...
            def __compare_unconditional(self, inputs:List[str], **kwargs) -> torch.LongTensor:
                ...
                # finalize probabilities:
                self._exp_logits[-1] = torch.concatenate(self._exp_logits[-1], dim=1).cpu()
            ...
            def __compare_conditional(self, inputs:List[str], outputs:Union[List[str], torch.LongTensor], batch_size:int=1, **kwargs) -> torch.LongTensor:
                ...
                if single_input and single_output:
                    self._exp_logits[-1] = torch.concatenate(self._exp_logits[-1], dim=0).transpose(0,1).cpu()

                else: self._exp_logits[-1] = torch.concatenate(self._exp_logits[-1], dim=1).cpu()
