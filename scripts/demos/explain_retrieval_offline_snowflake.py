...
# %%
encoder = ExplainableAutoModelForContextEncoding.from_pretrained(
    'Snowflake/snowflake-arctic-embed-l-v2.0',
    add_pooling_layer = False
).to('cuda' if torch.cuda.is_available() else 'cpu')

# Calculate similarity:
encoder.save_index(contexts, 32, dir='index_snowflake', output_attentions=True, output_hidden_states=True)
...
# Calculate similarity:
retriever('query: ' + query, 2, dir='index_snowflake', output_attentions=True, output_hidden_states=True)

#%% aGrad:
plot_importance_retriever(retriever, method='aGrad')
higlight_importance_retriever(retriever, method='aGrad', token_processor=lambda s: s.replace('▁', ' '))

#%% Grad x In:
plot_importance_retriever(retriever, method='gradIn')
higlight_importance_retriever(retriever, method='gradIn', token_processor=lambda s: s.replace('▁', ' '))
